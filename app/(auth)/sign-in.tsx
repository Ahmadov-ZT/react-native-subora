...
const SignIn = () => {
  ...
  const handleSubmit = async () => {
    posthog.capture("sign_in_pressed", {
      form_valid: formValid,
      email_present: emailAddress.length > 0,
    });
    if (!formValid) return;
      ...
      await signIn.finalize({
        navigate: ({ session, decorateUrl }) => {
          ...
          if (
            ...
            }, 100); // 100ms delay gives the navigator time to mount cleanly
          }
        },
      });
  ...
  const handleResendCode = async () => {
    posthog.capture("sign_in_resend_code_pressed");
    await signIn.mfa.sendEmailCode();
  };

  const handleStartOver = () => {
    posthog.capture("sign_in_start_over_pressed");
    signIn.reset();
  };
  ...
  const handleVerify = async () => {
    posthog.capture("sign_in_verify_pressed", {
      code_present: code.length > 0,
    });
